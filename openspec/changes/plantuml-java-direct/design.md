...
- **[Детерминизм между JDK-мажорами]** — advance-widths берутся из TTF (`hmtx`),
  обычно стабильны, но AWT-метрики между мажорами JDK теоретически могут дрейфовать.
  Митигируется пином Temurin 21 на CI; локально у разработчика JDK иной → остаточный
  риск, закрывается регенерацией эталона. Спайк: сравнить метрики Liberation Sans на
  двух JDK-мажорах.
...
- **[ditaa]** — `-Playout=smetana` для `@startditaa` безвреден (layout не участвует),
  но проверяется, что PNG-выход не изменился.

## Open Questions

- Ослаблять ли `test/README.md` полностью (убрать раздел «CI — источник истины») или
  оставить как процедуру на случай смены JDK-мажора? Решается после спайка по JDK.
