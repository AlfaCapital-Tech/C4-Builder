...
- [x] 5.2 Прогнать CI (Node 22/24) без graphviz: тесты зелёные, эталон совпадает
      → первый прогон выявил cross-vendor различие тесселяции глифов в class.svg
        (C4-диаграммы совпали байт-в-байт); эталон канонично пересоздан из CI-артефакта
        (Temurin 21), повторный прогон подтверждает зелёный
